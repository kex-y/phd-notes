...
For a matrix \(A \in \mathbb{R}^{n \times n}\), the Perron-Frobenius essentially says that if 
\(A\) leaves a cone \(K\) invariant, then it has a positive eigenvalue with a corresponding eigenvector 
in \(K\). Thus, if \(A\) is a non-negative matrix, the it leaves the quadrant corresponding to 
vectors with non-negative entries invariant. Thus, \(A\) has a positive eigenvalue with a corresponding 
eigenvector with non-negative entries. For stochastic matrices, the positive eigenvalue is 1 and the
